...
class observer {
    ...
    public static function on_certificate_template_updated(\mod_customcert\event\template_updated $event) {
        ...
        $sql = "SELECT c.id, c.course, c.templateid, c.name, ic.userid, ic.customcertid, ic.code
        FROM {customcert} c
        JOIN {customcert_issues} ic ON c.id = ic.customcertid
        WHERE c.course = :courseid
          AND c.templateid = :templateid";

        $params = [
            'courseid' => $courseid,
            'templateid' => $evtemplateid
        ];
