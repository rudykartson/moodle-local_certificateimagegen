...
Lorem ipsum dolor sit amet, consectetur adipiscing elit.'
    ...
    // Get saved courses from config
    $savedcourses = get_config('local_certimagegen', 'allowSectionCertificates');
    if (!empty($savedcourses)) {
        $savedcourses = explode(',', $savedcourses); // convert CSV to array
    } else {
        $savedcourses = [];
    }

    // Only get the courses that are selected
    if (empty($savedcourses)) {    $choices1 = ['0' => get_string('notselected', 'local_certimagegen')];
    } else {
        list($sql, $params) = $DB->get_in_or_equal($savedcourses);
        $selectedcourses = $DB->get_records_select('course', "id $sql", $params, 'fullname ASC');
    ...
    // Add multiselect with only selected courses
    $settings->add(new admin_setting_configmultiselect(
        'local_certimagegen/allowSectionCertificates',
        get_string('sectionCertificates', 'local_certimagegen'),
        '<small><a href="/local/certimagegen/select_course.php">' . get_string('selectcourseslink', 'local_certimagegen') . '</a></small><br>'.
        get_string('sectionCertificates_desc', 'local_certimagegen'), // show link text
        $savedcourses,  // pre-selected courses
        $choices1
    ));
