...
if (!empty($paged_issued)) {
    ...
    foreach ($paged_issued as $issue) {
        $certi = $DB->get_record('customcert', ['id' => $issue->customcertid], '*', MUST_EXIST);
        ...
        // Fallback: if image missing, try to generate for this user.
        $hasimage = local_certimagegen_ensure_issue_image((int)$issue->userid, (int)$certi->templateid, (string)$issue->code);

        $filename = 'certimage_' . $issue->code . '.jpg';
        $imageurl = moodle_url::make_pluginfile_url($contextsystem->id, 'local_certimagegen', 'content', 0, '/', $filename);
        $course = $DB->get_record('course', ['id' => $certi->course], '*', MUST_EXIST);

        // Section name (if cm is in a named section).
        $sectionname = $course->fullname;
        $section = $DB->get_record_sql("SELECT * FROM {course_sections} WHERE CONCAT(',', sequence, ',') LIKE ?", ['%,' . $cm->id . ',%']);
        if ($section && !empty($section->name)) {
            $sectionname = $section->name;
        }
        $courselabel = in_array((int)$course->id, $sectioncourseids, true) ? $sectionname : $course->fullname;

        $issuedcerts[] = [
            'cmid'         => (int)$cm->id,
            'courseid'     => (int)$course->id,
            'image_url'    => $imageurl->out(false),
            'course_label' => format_string($courselabel),
            'cert_title'   => format_string($certi->name),
            'download_url' => (new moodle_url('/mod/customcert/view.php', ['id' => $cm->id, 'downloadown' => 1]))->out(false),
            'share_url'    => (new moodle_url('/local/certimagegen/certificate.php', ['code' => $issue->code]))->out(false),
        ];
    }
}
...
// Inline JS to wire up sharing + copy + locked download refresh.
?>
<script>
document.querySelectorAll('.openShare').forEach(function(btn) {
  btn.addEventListener('click', function(event) {
    var urlInput = btn.closest('.bottombtn').querySelector('.shareURL');
    ...
    if (navigator.share) {
      ...
      var message = attachmsg && attachmsg.value ? attachmsg.value : 'Check this out:';

      var fbook = document.getElementById('fbook');
      var linked = document.getElementById('linked');
      var whatsapp = document.getElementById('whatsapp');
      var twitterx = document.getElementById('twitterx');
      if (fbook) fbook.setAttribute('href', 'https://www.facebook.com/sharer.php?u=' + encodedUrl);
      if (linked) linked.setAttribute('href', 'https://www.linkedin.com/sharing/share-offsite/?url=' + encodedUrl);
      if (whatsapp) whatsapp.setAttribute('href', 'https://wa.me/?text=' + encodeURIComponent(message) + '%20' + encodedUrl);
      if (twitterx) twitterx.setAttribute('href', 'https://twitter.com/share?text=' + encodeURIComponent(message) + '&url=' + encodedUrl);
...
//     var urlInput = this.closest('.bottombtn').querySelector('.shareURL');
//     var url = urlInput ? urlInput.value : '';
//     var encodedUrl = encodeURIComponent(url);
//     var attachmsg = document.getElementById('attachmsg');
//     var message = attachmsg && attachmsg.value ? attachmsg.value : 'Check this out:';
...
// Handle "Download" on locked cards to show loader and refresh.
document.querySelectorAll('.download-locked').forEach(function(a){
  a.addEventListener('click', function(e){
    // Let the download happen, but also update UI.
    var cmid = this.getAttribute('data-cmid');
    var imgdata = document.getElementById('imgdata' + cmid);
    if (imgdata) {
      imgdata.innerHTML = '<div class="certimg" style="height:240px;display:grid;justify-content:center;">' +
        '<div class="loader"></div><span>Generating image…</span></div>';
